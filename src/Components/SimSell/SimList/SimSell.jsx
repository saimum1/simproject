...
const SimSell = () => {
    ...
    return (
        <div  className="flex justify-center h-full w-full items-center md:items-start bg-[#303038]  rounded-[7px]" >
            <SaveSell simForEdit={simForEdit} isOpen={isOpen} onClose={onClose} data={selecteditem} action={actiontype} GetSimList={GetSimList}/>
                ...
                <Table className="mt-5">
                    ...
                    <TableBody  >
                        ...
                        {simList?.filter(item=>item.status === 'available').map((item,index) => (
                            <TableRow key={item.id} style={{borderColor:'#595959',maxHeight:'auto',height:'7rem'}}>
                                <TableCell>
                                ...
                                <TableCell>
                                    <div style={{position:'relative',width:"100%" ,backgroundColor:'',cursor:'pointer',display:'flex',justifyContent:'center',alignItems:'center',flexDirection:'column'}} onClick={()=>{setshowedit(index === showeditindex? false :true);setshoweditindex(index === showeditindex ? null :index);setselecteditem(item)}}>
                                        ...
                                        {showedit &&  <div style={{position:'absolute',width:'8rem',right:'50%',top:'1%',zIndex:'9999999' ,height:'fit-content',display:( index === showeditindex) ?'flex':'none'}}>
